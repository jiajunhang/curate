...
const TemplateDashboard = ({ selectedQuiz, results }) => {
  ...
  const arr = [0.17835878, 1.29954448, -0.63545942, -0.9160703, 0.90438934,
  ...
  const binData = () => {
    const histGen = d3.bin().domain([-3, 3]).thresholds(20);

    const bins = histGen(arr);
    const mapped = [];
    ...
    const data = results.map(r => (
      {
        ...
        numCorrect: r.summary.total_correct
      }));
      ...
      <Grid container alignItems="stretch" direction="row" spacing={2}>
        ...
        <Grid item xs={6}>
          <Paper sx={{ p: 2 }}>
            ...
            >
              <CartesianGrid />
              <XAxis label="ability" type="number" domain={[-3, 3]} dataKey="ability" />
              <YAxis label="ability" type="number" domain={[-3, 3]} dataKey="ability" />
